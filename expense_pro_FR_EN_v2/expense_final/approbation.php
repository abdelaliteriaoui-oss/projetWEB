<?php
/**
 * ExpensePro - Approbation Admin
 * Page for admins to approve validated expense requests
 */
...
$pageTitle = 'Approbation des demandes';

// Récupérer les demandes à approuver (validées par manager)
$pendingQuery = $pdo->query("
    ...
    GROUP BY d.id
    ORDER BY d.date_validation_manager ASC
");
...
$stats = [
    ...
    'approved_today' => $pdo->query("SELECT COUNT(*) FROM demandes WHERE statut = 'approuvee_admin' AND DATE(date_validation_admin) = CURDATE()")->fetchColumn(),
    'rejected_today' => $pdo->query("SELECT COUNT(*) FROM demandes WHERE statut = 'rejetee_admin' AND DATE(date_validation_admin) = CURDATE()")->fetchColumn(),
    'total_month' => $pdo->query("SELECT COALESCE(SUM(montant_total), 0) FROM demandes WHERE statut = 'approuvee_admin' AND MONTH(date_validation_admin) = MONTH(NOW())")->fetchColumn()
];
...
<div class="page-header">
    <div class="page-header-content">
        <h2>Approbation des demandes</h2>
        <p>Demandes validées par les managers en attente d'approbation finale</p>
    </div>
</div>
...
<div class="stats-grid" style="margin-bottom: 24px;">
    ...
    <div class="stat-card">
        ...
        <div class="stat-content">
            ...
            <div class="stat-label">Approuvé ce mois</div>
        </div>
    </div>
</div>
...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">
            ...
            Demandes à approuver (<?= $pendingCount ?>)
        </h3>
    </div>
    <div class="card-body" style="padding: 0;">
        ...
        <div class="empty-state">
            ...
            <p class="empty-state-text">Aucune demande en attente d'approbation</p>
        </div>
        <?php else: ?>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="font-weight: 500;"><?= htmlspecialchars($demande['objet']) ?></div>
                            <div style="font-size: 12px; color: var(--gray-500);"><?= htmlspecialchars($demande['categories'] ?? 'N/A') ?></div>
                        </td>
                        <td style="font-weight: 600; font-size: 16px;"><?= formatMoney($demande['montant_total']) ?></td>
                        <td>
                            <div style="font-size: 13px;">
                                <?= htmlspecialchars(($demande['manager_prenom'] ?? '') . ' ' . ($demande['manager_nom'] ?? '')) ?>
                            </div>
                        </td>
                        <td>
                            <span title="<?= $demande['date_validation_manager'] ? date('d/m/Y H:i', strtotime($demande['date_validation_manager'])) : '' ?>">
                                <?= $demande['date_validation_manager'] ? timeAgo($demande['date_validation_manager']) : 'N/A' ?>
                            </span>
                        </td>
                        <td>
                            <div class="table-actions">
                                ...
                                <button class="btn btn-success btn-icon" onclick="approuverDemande(<?= $demande['id'] ?>)" title="Approuver">
...
<!-- Modal Approbation -->
<div class="modal" id="approbationModal">
    <div class="modal-content">
        <div class="modal-header">
            <h3 class="modal-title">Approuver la demande</h3>
            <button class="modal-close" onclick="closeModal('approbationModal')">&times;</button>
        </div>
        <form action="traiter_demande.php" method="POST">
            <input type="hidden" name="demande_id" id="approbation_demande_id">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-ghost" onclick="closeModal('approbationModal')">Annuler</button>
...
function approuverDemande(id) {
    document.getElementById('approbation_demande_id').value = id;
    document.getElementById('approbationModal').classList.add('active');
}
